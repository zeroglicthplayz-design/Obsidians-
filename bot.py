    ...
    async def init_guild(self, guild_id):
        ...
        if not _os.path.exists(path):
            default = {
                ...
                "welcome": {"enabled": True, "message": "Welcome {user} to **{server}**! You are member **#{count}**.", "dm_message": "Thanks for joining {server}! Read the rules."},
...
class ObsidianEmbeds:
    ...
    def success(self, title, description=None):
        return self._base(f"✅ {title}", description, Config.SUCCESS_COLOR)
    def error(self, title, description=None):
        return self._base(f"❌ {title}", description, Config.ERROR_COLOR)
    def warning(self, title, description=None):
        return self._base(f"⚠️ {title}", description, Config.WARNING_COLOR)
    def info(self, title, description=None):
        return self._base(f"ℹ️ {title}", description, Config.INFO_COLOR)
    def shield(self, title, description=None):
        return self._base(f"🛡️ {title}", description, Config.PRIMARY_COLOR)
    def ticket(self, title, description=None):
        return self._base(f"🎫 {title}", description, Config.INFO_COLOR)
    def payment(self, title, description=None):
        return self._base(f"💰 {title}", description, Config.SUCCESS_COLOR)
    def automod(self, title, description=None):
        return self._base(f"🔨 {title}", description, Config.WARNING_COLOR)
    ...
    def stock_listing(self, name, price, description, seller, stock):
        embed = discord.Embed(title=f"🛒 {name}", description=description, color=Config.GOLD_COLOR, timestamp=datetime.utcnow())
...
class AutoMod(commands.Cog):
    ...
    async def _check_spam(self, message, config):
        self.message_cache[message.author.id].append({"content": message.content, "time": datetime.utcnow(), "channel": message.channel.id})
    ...
    async def automod_stats(self, interaction: discord.Interaction):
        config = await self.db.get_automod(interaction.guild_id)
        embed = self.embeds.info("AutoMod Config", "Current settings:")
        for k, v in config.items(): embed.add_field(name=k, value=str(v), inline=True)
        await interaction.response.send_message(embed=embed, ephemeral=True)

# ===== WELCOME COG =====
class Welcome(commands.Cog):
    def __init__(self, bot): self.bot = bot; self.db = bot.db; self.embeds = bot.embeds
    ...
    async def on_member_remove(self, member):
        ...
        if ch:
            try: await ch.send(embed=discord.Embed(description=config.get("goodbye_message", "👋 {user} left.").format(user=member.mention, username=member.name, server=member.guild.name), color=Config.ERROR_COLOR))
            except: pass
...
class TicketCloseView(View):
    ...
    async def close_button(self, interaction: discord.Interaction, button: Button):
        if not interaction.channel.name.startswith("ticket-"): return await interaction.response.send_message("Not a ticket!", ephemeral=True)
...
class TicketCreateView(View):
    ...
    async def create_button(self, interaction: discord.Interaction, button: Button):
        ...
        if len(user_tickets) >= max_tickets: return await interaction.response.send_message(embed=discord.Embed(title="❌ Limit Reached", description=f"Max {max_tickets} tickets", color=Config.ERROR_COLOR), ephemeral=True)
...
class TicketModal(Modal):
    def __init__(self, bot):
        ...
        self.add_item(self.reason); self.add_item(self.category)
    async def on_submit(self, interaction: discord.Interaction):
        config = await self.bot.db.get_tickets(interaction.guild_id)
        category = interaction.guild.get_channel(config.get("category")) if config.get("category") else None
        overwrites = {interaction.guild.default_role: discord.PermissionOverwrite(read_messages=False), interaction.user: discord.PermissionOverwrite(read_messages=True, send_messages=True), interaction.guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_channels=True)}
        ...
        embed = discord.Embed(title=f"🎫 {self.category.value}", description=f"**User:** {interaction.user.mention}\n**Reason:** {self.reason.value}", color=Config.INFO_COLOR, timestamp=datetime.utcnow())
...
class Tickets(commands.Cog):
    ...
    async def close_ticket(self, interaction: discord.Interaction):
        if not interaction.channel.name.startswith("ticket-"): return await interaction.response.send_message(embed=self.embeds.error("Not a ticket", "Use this in a ticket channel."), ephemeral=True)
...
class Stocks(commands.Cog):
    ...
    async def _save_listing(self, guild_id, listing):
        ...
        for i, l in enumerate(listings):
            if l["id"] == listing["id"]: listings[i] = listing; break
        else: listings.append(listing)
    ...
    async def listing_view(self, interaction: discord.Interaction, listing_id: int):
        ...
        if not listing: return await interaction.response.send_message(embed=self.embeds.error("Not Found", f"Listing `#{listing_id}` doesn't exist."), ephemeral=True)
    ...
    async def listing_list(self, interaction: discord.Interaction, category: Optional[str] = None):
        ...
        if not listings: return await interaction.response.send_message(embed=self.embeds.info("No Listings", "No listings found."), ephemeral=True)
        ...
        for l in listings[:25]:
            ...
            embed.add_field(name=f"#{l['id']} - {l['name']}", value=f"💰 `${l['price']:.2f}` | 📦 `{l['stock']}` | 👤 {seller.name if seller else 'Unknown'}", inline=False)
        await interaction.response.send_message(embed=embed)
    ...
    async def listing_delete(self, interaction: discord.Interaction, listing_id: int):
        listings = await self.db.get(interaction.guild_id, "listings", [])
        listing = next((l for l in listings if l["id"] == listing_id), None)
        if not listing or listing["seller_id"] != interaction.user.id: return await interaction.response.send_message(embed=self.embeds.error("Denied", "Not your listing or doesn't exist."), ephemeral=True)
...
class Payments(commands.Cog):
    ...
    async def payment_add(self, interaction: discord.Interaction, method: str, info: str):
        ...
        if str(interaction.user.id) not in payments: payments[str(interaction.user.id)] = {}
    ...
    async def payment_view(self, interaction: discord.Interaction):
        ...
        if not methods: return await interaction.response.send_message(embed=self.embeds.info("No Methods", "Use /payment-add to add one."), ephemeral=True)
        embed = self.embeds.payment("Your Methods", "Visible to buyers:")
        for m, i in methods.items(): embed.add_field(name=m, value=f"`{i}`", inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def payment_view_user(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not methods: return await interaction.response.send_message(embed=self.embeds.error("No Methods", f"{user.mention} has none set up."), ephemeral=True)
...
class AntiNuke(commands.Cog):
    def __init__(self, bot): self.bot = bot; self.db = bot.db; self.embeds = bot.embeds; self.tracker = ActionTracker(); self.punished = {}
    ...
    async def _punish(self, member, reason, config):
        ...
        try:
            if punishment == "ban": await member.ban(reason=f"[Anti-Nuke] {reason}")
            elif punishment == "kick": await member.kick(reason=f"[Anti-Nuke] {reason}")
...
class Moderation(commands.Cog):
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: Optional[str] = "No reason"):
        if member.top_role >= interaction.user.top_role: return await interaction.response.send_message(embed=self.embeds.error("Hierarchy", "Can't kick this member."), ephemeral=True)
    ...
    async def ban(self, interaction: discord.Interaction, member: discord.Member, reason: Optional[str] = "No reason", delete_days: Optional[int] = 0):
        if member.top_role >= interaction.user.top_role: return await interaction.response.send_message(embed=self.embeds.error("Hierarchy", "Can't ban this member."), ephemeral=True)
    ...
    async def timeout(self, interaction: discord.Interaction, member: discord.Member, duration: str, reason: Optional[str] = "No reason"):
        if member.top_role >= interaction.user.top_role: return await interaction.response.send_message(embed=self.embeds.error("Hierarchy", "Can't timeout this member."), ephemeral=True)
        units = {"s": 1, "m": 60, "h": 3600, "d": 86400, "w": 604800}
        try:
            seconds = int(duration[:-1]) * units[duration[-1].lower()]
            seconds = min(seconds, 2419200)
        except: return await interaction.response.send_message(embed=self.embeds.error("Invalid", "Use: `30m`, `1h`, `1d`, `1w`"), ephemeral=True)
    ...
    async def warn(self, interaction: discord.Interaction, member: discord.Member, reason: str):
        ...
        try: await member.send(embed=discord.Embed(title=f"⚠️ Warning from {interaction.guild.name}", description=f"**Reason:** {reason}\n**Total:** `{count}`", color=Config.WARNING_COLOR))
        except: pass
    ...
    async def warnings(self, interaction: discord.Interaction, member: discord.Member):
        ...
        if not warns: return await interaction.response.send_message(embed=self.embeds.info("Clean", f"{member.mention} has no warnings."), ephemeral=True)
    ...
    async def lock(self, interaction: discord.Interaction, channel: Optional[discord.TextChannel] = None, reason: Optional[str] = "No reason"):
        ...
        if ch != interaction.channel: await interaction.response.send_message(embed=self.embeds.success("Locked", f"{ch.mention} locked."), ephemeral=True)
    ...
    async def unlock(self, interaction: discord.Interaction, channel: Optional[discord.TextChannel] = None):
        ...
        if ch != interaction.channel: await interaction.response.send_message(embed=self.embeds.success("Unlocked", f"{ch.mention} unlocked."), ephemeral=True)
...
class Utility(commands.Cog):
    ...
    async def help_cmd(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🛡️ Moderation", value="`/kick` `/ban` `/timeout` `/warn` `/warnings` `/clear-warns` `/purge` `/slowmode` `/lock` `/unlock`", inline=False)
        ...
        embed.add_field(name="🛡️ Anti-Nuke", value="`/antinuke-config` `/antinuke-status`", inline=False)
        embed.add_field(name="👋 Welcome", value="`/welcome-config` `/welcome-test` `/welcome-toggle`", inline=False)
        embed.add_field(name="⚙️ Utility", value="`/help` `/ping` `/server-info` `/user-info` `/bot-info`", inline=False)
    ...
    async def server_info(self, interaction: discord.Interaction):
        g = interaction.guild
        embed = discord.Embed(title=f"📊 {g.name}", color=Config.PRIMARY_COLOR)
    ...
    async def user_info(self, interaction: discord.Interaction, member: Optional[discord.Member] = None):
        ...
        embed = discord.Embed(title=f"👤 {m}", color=m.color if m.color != discord.Color.default() else Config.PRIMARY_COLOR)
    ...
    async def bot_info(self, interaction: discord.Interaction):
        ...
        days, hours = divmod(int(uptime.total_seconds()) // 3600, 24)
        minutes = (int(uptime.total_seconds()) % 3600) // 60
        embed = discord.Embed(title="🔮 Obsidian Marketplace Bot", description="Premium marketplace bot.", color=Config.PRIMARY_COLOR)
    ...
    async def set_logs(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await interaction.response.send_message(embed=self.embeds.success("Logs Set", f"Logs: {channel.mention}"), ephemeral=True)
...
class Owner(commands.Cog):
    ...
    async def reload_cog(self, ctx, cog: str):
        try: await self.bot.reload_extension(cog); await ctx.send(embed=self.bot.embeds.success("Reloaded", f"`{cog}`"))
        except Exception as e: await ctx.send(embed=self.bot.embeds.error("Error", f"```{e}```"))
    @commands.command(name="sync")
    @commands.is_owner()
    async def sync_cmd(self, ctx, guild_id: str = None):
        if guild_id:
            synced = await self.bot.tree.sync(guild=discord.Object(id=int(guild_id)))
            await ctx.send(embed=self.bot.embeds.success("Synced", f"`{len(synced)}` commands to guild."))
        else:
            synced = await self.bot.tree.sync()
            await ctx.send(embed=self.bot.embeds.success("Synced", f"`{len(synced)}` global commands."))
    @commands.command(name="eval")
    @commands.is_owner()
    async def eval_cmd(self, ctx, *, code: str):
        import textwrap, traceback, io
